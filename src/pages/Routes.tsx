...
export default function Routes() {
  ...
  return (
    <div className="card">
      ...
      <div className="grid">
        <label>Máx paradas/rota</label>
        <input value={String(maxStops)} onChange={(e) => setMaxStops(Number(e.target.value || 5))} />

        <label>Raio (km)</label>
        <input value={String(radiusKm)} onChange={(e) => setRadiusKm(Number(e.target.value || 1.2))} />
      </div>

      <div className="list" style={{ marginTop: 12 }}>
        {routes.map((r) => {
          const stops = r.delivery_ids.map((id) => byId.get(id)).filter(Boolean) as DeliveryRow[];
          const coords = stops
          ...
          return (
            <div key={r.id} className="item col">
              ...
              {url ? (
                ...
                <span className="muted">Sem lat/lng nas entregas</span>
              )}
            </div>
          );
        })}
