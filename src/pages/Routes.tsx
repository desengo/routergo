...
import { getCompanySettings, DemandMode } from "../lib/companySettings";
...
export default function Routes() {
  ...
  // 🔹 modo atual (vem de companySettings)
  ...
  async function loadAll() {
    ...
    try {
      ...
      const r = await supabase
        .from("routes")
        .select(
          "id,name,status,delivery_ids,stops,total_est_km,concluded_at,concluded_by,created_at"
        )
        .eq("user_id", userId)
        .order("created_at", { ascending: false });
  ...
  // 🔹 carrega modo de demanda (uma vez)
  useEffect(() => {
  ...
  // 🔥 primeira carga + tentativa de auto gerar (uma vez)
  useEffect(() => {
    async function init() {
      try {
  ...
  // ✅ se tiver delivery_id, monta label com endereço (sempre)
